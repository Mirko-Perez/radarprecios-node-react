...
const CentroPrecio = () => {
  ...
  const handleSubmit = async e => {
    ...
    setError('');
    setSuccess('');
    if (!brandId || !productId || !storeId || !price) return setError('Todos los campos son obligatorios');
          ...
          <>
            ...
            <div style={{ maxHeight: 200, overflowY: 'auto', marginBottom: 10 }}>
              {filteredProducts.map(product => (
                <div key={product.product_id} style={{ display:'flex',alignItems:'center',gap:10,marginBottom:8,background: productId===String(product.product_id)?'#e3f2fd':'#fff',borderRadius:6,padding:6,cursor:'pointer' }} onClick={() => setProductId(String(product.product_id))}>
                  {product.imagen && <img src={product.imagen.startsWith('/api/images/') ? product.imagen : product.imagen.startsWith('/images/') ? `/api${product.imagen}` : `/api/images/${product.imagen.replace(/^.*[\\/]/, '')}`} alt={product.product_name} style={{ maxWidth:60,maxHeight:60,borderRadius:8 }} />}
