...
const CentroPrecio = () => {
  ...
  useEffect(() => {
    ...
    setProducts([]);
    if (brandId) {
      fetch(`${API_URL}/prices/products/${brandId}/${regionId}`)
        .then((res) => res.json())
    ...
    <div className="andes-container">
      ...
      <form className="andes-form" onSubmit={handleSubmit}>
        <Select
          ...
          value={brands
            .filter((b) => String(b.brand_id) === String(brandId))
            .map((b) => ({ value: b.brand_id, label: b.brand_name }))}
          onChange={(o) => setBrandId(o ? o.value : "")}
          placeholder="Selecciona marca..."
          isClearable
          styles={{
            container: (base) => ({ ...base, width: "100%", marginBottom: 10 }),
          }}
        />
        {brandId && (
          <>
              ...
              {filteredProducts.map((product) => (
                ...
                >
                  {product.imagen && (
                    <img
                      src={
                        product.imagen.startsWith("/api/images/")
                          ? product.imagen
