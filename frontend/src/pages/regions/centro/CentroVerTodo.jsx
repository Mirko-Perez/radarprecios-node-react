...
const CentroVerTodo = () => {
  ...
  return (
    <div className="andes-container andes-table-container">
      <button className="andes-btn back-btn" style={{ marginBottom: 20 }} onClick={() => navigate(-1)}>Volver atrás</button>
      <h2>Comparativo de precios Centro</h2>
      <input className="andes-search" type="text" placeholder="Buscar producto" value={search} onChange={e => setSearch(e.target.value)} />
      <table className="andes-table">
        <thead>
          <tr>
            <th className="andes-th andes-th-product">Producto</th>
            {stores.map(store => <th key={store.store_id} className="andes-th andes-th-store">{store.store_name}</th>)}
        ...
        <tbody>
          {Object.entries(groups).map(([groupId, group]) => (
            <React.Fragment key={groupId}>
              <tr className="andes-group-row"><td colSpan={1 + stores.length + 2} className="andes-td-group">{group.name}</td></tr>
              {Object.entries(group.products).filter(([productName]) => filterProduct(productName)).map(([productName, product]) => (
                <React.Fragment key={productName}>
                  <tr className="andes-product-row">
                    <td colSpan={1 + stores.length + 2} className="andes-td-product-header">
                      {product.imagen && <img src={product.imagen.startsWith('/api/images/') ? product.imagen : product.imagen.startsWith('/images/') ? `/api${product.imagen}` : `/api/images/${product.imagen.replace(/^.*[\\/]/, '')}`} alt={productName} className="andes-product-img" />}
                  ...
                  {Object.entries(product.brands).map(([brandId, brandObj]) => {
                    ...
                    return (
                      <tr key={brandId}>
                        ...
                        {stores.map(store => <td key={store.store_id} className={brandObj.prices[store.store_id] ? 'andes-td-price' : 'andes-td-empty'}>{brandObj.prices[store.store_id] !== undefined && brandObj.prices[store.store_id] !== null ? Number(brandObj.prices[store.store_id]).toLocaleString('es-VE', { minimumFractionDigits: 2 }) : '-'}</td>)}
                        <td className="andes-td-min">{min !== null ? Number(min).toLocaleString('es-VE', { minimumFractionDigits: 2 }) : '-'}</td>
                        <td className="andes-td-max">{max !== null ? Number(max).toLocaleString('es-VE', { minimumFractionDigits: 2 }) : '-'}</td>
                      </tr>
                    );
                  })}
                </React.Fragment>
