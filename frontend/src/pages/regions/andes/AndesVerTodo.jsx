...
const AndesVerTodo = () => {
  ...
  // Process each row only if data exists
  if (Array.isArray(data) && data.length > 0) {
    data.forEach(row => {
      // Process stores
      if (row.store_id && !storeMap[row.store_id]) {
        storeMap[row.store_id] = { 
          store_id: row.store_id, 
          store_name: row.store_name || `Tienda ${row.store_id}` 
        };
      ...
      if (row.group_id) {
        if (!groups[row.group_id]) {
          groups[row.group_id] = { 
            name: row.group_name || `Grupo ${row.group_id}`, 
            products: {} 
          };
        }
        
        const group = groups[row.group_id];
        const productName = row.product_name || `Producto ${row.product_id}`;
        ...
        const product = group.products[productName];
        
        if (row.brand_id) {
          if (!product.brands[row.brand_id]) {
            product.brands[row.brand_id] = { 
              brand: row.brand_name || `Marca ${row.brand_id}`, 
              prices: {} 
            };
          }
          
          const brand = product.brands[row.brand_id];
          
          // Only add price if it exists
          if (row.price_amount !== undefined && row.price_amount !== null) {
            brand.prices[row.store_id] = row.price_amount;
          }
        }
      }
    });
  ...
  return (
    <div className="andes-container andes-table-container">
      ...
      <h2>Comparativo de precios Andes</h2>
      <input
        type="text"
