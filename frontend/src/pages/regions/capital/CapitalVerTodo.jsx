...
const CapitalVerTodo = () => {
  ...
  return (
    <div className="andes-container andes-table-container">
      ...
      <h2>Comparativo de precios Capital</h2>
      <input type="text" placeholder="Buscar producto" value={search} onChange={e => setSearch(e.target.value)} className="andes-search" />
                ...
                <React.Fragment key={productName}>
                  <tr className="andes-product-row">
                    <td colSpan={1 + stores.length + 2} className="andes-td-product-header">
                      {product.imagen && (
                        <img src={product.imagen.startsWith('/api/images/') ? product.imagen : product.imagen.startsWith('/images/') ? `/api${product.imagen}` : `/api/images/${product.imagen.replace(/^.*[\\/]/, '')}`} alt={productName} className="andes-product-img" />
                      )}
                  ...
                  {Object.entries(product.brands).map(([brandId, brandObj]) => {
                    ...
                    return (
                      <tr key={brandId}>
                        ...
                        {stores.map(store => (
                          <td key={store.store_id} className={brandObj.prices[store.store_id] ? 'andes-td-price' : 'andes-td-empty'}>
                            {brandObj.prices[store.store_id] !== undefined && brandObj.prices[store.store_id] !== null ? Number(brandObj.prices[store.store_id]).toLocaleString('es-VE', { minimumFractionDigits: 2 }) : '-'}
                          </td>
                        ))}
