...
import { FaSearch, FaMapMarkerAlt, FaLayerGroup } from 'react-icons/fa';
...
const Dashboard = () => {
  ...
  const processBrandsForChart = (brands) => {
    ...
    const sortedBrands = [...brands].sort((a, b) => b.average_price - a.average_price);

    // Encontrar la marca Fritz si existe
    const fritzBrand = sortedBrands.find(brand =>
      brand.brand && brand.brand.toLowerCase().includes('fritz')
    );
  ...
  const fetchBrandStats = async (regionId = '', groupId = '') => {
    ...
    // Si es 'all', combinar resultados de todas las regiones
    if (regionId === 'all' && Array.isArray(regions) && regions.length > 0) {
      try {
        ...
        for (const region of regions) {
          ...
          params.append('regionId', region.region_id);
          if (groupId && groupId !== 'all') {
            const group = groups.find(g => g.group_id === groupId);
            if (group) {
              params.append('groupId', encodeURIComponent(group.group_name));
            }
          }
          const url = `${API_URL}/brands/stats?${params.toString()}`;
          ...
          if (Array.isArray(response.data)) {
            // Agregar campo de región a cada marca
            allBrands = allBrands.concat(response.data.map(b => ({ ...b, region: region.region_name })));
          }
        }
        setBrandStats(allBrands);
        // Para el gráfico, mostrar el top/bottom de todas las marcas combinadas
        processBrandsForChart(allBrands);
      } catch (error) {
        setBrandStats([]);
        setTopAndBottomBrands([]);
      }
    ...
    try {
      ...
      if (!Array.isArray(response?.data)) {
        throw new Error('Formato de respuesta inesperado para estadísticas de marcas');
      }
      // Buscar el nombre de la región
      let regionName = 'Sin región';
      if (Array.isArray(regions)) {
        const regionObj = regions.find(r => r.region_id == regionId);
        if (regionObj) regionName = regionObj.region_name;
      }
      const validBrands = response.data.filter(brand =>
        brand &&
        brand.brand &&
        (brand.average_price !== null && brand.average_price !== undefined)
      ).map(brand => ({ ...brand, region: regionName }));
      setBrandStats(validBrands);
      processBrandsForChart(validBrands);
    } catch (error) {
      setBrandStats([]);
      setTopAndBottomBrands([]);
    }
  ...
  const fetchProductGroupAverages = async (regionId = 'all') => {
    try {
      ...
      const response = await axios.get(`${API_URL}/prices/averages-by-product-group?${params.toString()}`);
      ...
      const processedData = Object.values(groupedData).map(item => {
        const average_price = item.count > 0 ? (item.total_price / item.count) : 0;

        return {
          ...item,
          region_id: regionId,
          region: regions.find(r => r.region_id == regionId)?.region_name || 'Todas las regiones',
  ...
  const fetchRegionAverages = async (regionId = 'all') => {
    ...
    try {
      ...
      const processedData = products.map(item => {
        ...
        const productRegion = allRegions.find(r => r.region_id == item.region_id);
        const regionName = productRegion ? productRegion.region_name : 'Sin región';
        return {
          ...item,
          region: regionName,
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        } else if (regionsResponse.data && Array.isArray(regionsResponse.data.regions)) {
          regionsData = regionsResponse.data.regions;
        }

        const formattedRegions = regionsData.map(region => ({
          region_id: String(region.region_id || region.id || ''),
          region_name: String(region.region_name || region.name || 'Sin nombre')
        }));
  ...
  useEffect(() => {
    ...
    return () => {
      isMounted = false;
      controller.abort('Component unmounted or region changed, canceling requests');
    };
  ...
  const fetchGroups = async (regionId) => {
    ...
    try {
      ...
      const response = await axios.get(`${API_URL}/prices/average-by-region?region_id=${regionId}`, {
        timeout: 10000,
        headers: {
          'Cache-Control': 'no-cache',
          'Pragma': 'no-cache'
      ...
      const groupsWithPrices = response.data
        .filter(item => {
          ...
          const hasValidPrice = item.precio_promedio !== null &&
      ...
      setGroups(prevGroups => {
        ...
        const groupsChanged = JSON.stringify(prevGroups) !== JSON.stringify(formattedGroups);
      ...
      if (currentGroupId && currentGroupId !== 'all') {
        const groupExists = formattedGroups.some(g => g.group_id === currentGroupId);
        if (!groupExists) {
          setSelectedGroup('all');
        ...
        if (selectedRegion === 'all') {
          ...
          const fetchPromises = regions.map(async (region) => {
            ...
            try {
              ...
              console.error('Error fetching data for region', region.region_id, err);
              return [];
            }
          });
          ...
          const response = await axios.get(`${API_URL}/overview/${selectedRegion}`);

          if (response.data) {
            if (Array.isArray(response.data) && response.data.length > 0) {
              processProductData(response.data, selectedRegion);
            } else {
              setProductData([]);
            }
          } else {
  ...
  const processProductData = (data, regionId) => {
    try {
        ...
        try {
          ...
          if (!productGroups[productKey]) {
            productGroups[productKey] = {
              ...
              region_id: regionId,
              region_name: regions.find(r => r.region_id === regionId)?.region_name || 'Sin región',
              prices: []
            };
          }
      ...
      const processedData = Object.values(productGroups).map(product => {
        const validPrices = product.prices.filter(p => !isNaN(p.price_amount));
        const avgPrice = validPrices.length > 0
          ? validPrices.reduce((sum, p) => sum + p.price_amount, 0) / validPrices.length
          : 0;
  ...
  const fetchOverviewData = async (regionId) => {
    ...
    try {
      ...
      if (response.data) {
        if (Array.isArray(response.data) && response.data.length > 0) {
          processProductData(response.data, regionId);
        } else {
          setProductData([]);
        }
      } else {
  ...
  const getGroupAverages = (data, regions) => {
    ...
    data.forEach(item => {
      ...
      let regionName = 'Sin región';
      if (Array.isArray(regions)) {
        const region = regions.find(r => r && (r.region_id == regionId || r.id == regionId));
        if (region) {
          regionName = region.region_name || region.name || `Región ${regionId}`;
        }
      }
    ...
    groupedByGroup.forEach(group => {
      group.products.forEach(product => {
        if (product.prices.length > 0) {
          const avgPrice = product.prices.reduce((sum, price) => sum + price, 0) / product.prices.length;
  ...
  const getGroupedPricesByRegion = useMemo(() => {
    ...
    const groupedByRegionAndGroup = regionAverages.reduce((acc, item) => {
      ...
      if (item.product_id && item.product_name && item.average_price !== undefined) {
  ...
  const filteredGroupedPrices = useMemo(() => {
    ...
    if (selectedRegion && selectedRegion !== 'all') {
      filtered = filtered.filter(item =>
        String(item.region_id) === String(selectedRegion)
      );
    }
  ...
  const filterRegionAverages = () => {
    ...
    const selectedGroupInfo = selectedGroup && selectedGroup !== 'all'
    ...
    try {
      ...
      if (selectedRegion && selectedRegion !== '') {
        ...
        if (selectedRegion === 'all') {
          ...
          result = result.filter(item =>
            item.region_id !== undefined && item.region_id !== null
          );
        } else {
          result = result.filter(item =>
            item.region_id && item.region_id.toString() === selectedRegion.toString()
          );
        }
      }
      ...
      if (selectedGroup && selectedGroup !== '' && selectedGroup !== 'all') {
        result = result.filter(item =>
          item.group_id && item.group_id.toString() === selectedGroup.toString()
        );
      }
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-filters">
        <div className="filter-group region-buttons">
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', minHeight: 40 }}>
            <span style={{ fontWeight: 500, fontSize: '1.08em', display: 'flex', alignItems: 'center', minWidth: 80 }}>
              Región
              <FaMapMarkerAlt style={{ marginLeft: 6, fontSize: 18, color: '#1976d2', verticalAlign: 'middle' }} />
            </span>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              <button
                type="button"
                className={`region-btn${selectedRegion === 'all' ? ' selected' : ''}`}
                onClick={() => handleRegionChange({ target: { value: 'all' }, preventDefault: () => { } })}
                disabled={loading}
              >
                Todas las regiones
              </button>
              {Array.isArray(regions) && regions.map(region => (
                <button
                  ...
                  onClick={() => handleRegionChange({ target: { value: region.region_id }, preventDefault: () => { } })}
                  disabled={loading}
                >
          ...
          {loading && <span className="loading-indicator">Cargando regiones...</span>}
      ...
      <div className="dashboard-grid">
        <div className="dashboard-grid-row">
          ...
          <div className="dashboard-grid-item dashboard-table">
            ...
            <div className="table-responsive" style={{ flex: 1, overflow: 'auto', maxHeight: 350 }}>
              <table>
                <thead>
                  <tr>
                    <th>Grupo</th>
                  ...
                  {productGroupAverages.length === 0 ? (
                    <tr>
                      <td colSpan="3" className="no-data">
                        {loading ? 'Cargando datos...' : 'No se encontraron precios promedios con los filtros seleccionados'}
                      </td>
                    </tr>
                  ) : (
                    [...productGroupAverages].sort((a, b) => {
                      if (a.product_name < b.product_name) return -1;
                      if (a.product_name > b.product_name) return 1;
                      if (a.group_name < b.group_name) return -1;
                      if (a.group_name > b.group_name) return 1;
                      ...
                      <tr key={`item-${index}`}
                        style={{ borderLeft: `4px solid ${index % 2 === 0 ? '#f0f7ff' : '#e6f2ff'}` }}>
                        <td>{item.group_name || 'Sin grupo'}</td>
                        <td>{item.product_name || 'Sin nombre'}</td>
                        <td style={{ textAlign: 'right' }}>{item.average_price > 0 ? `Bs.S ${parseFloat(item.average_price).toFixed(2)}` : 'N/A'}</td>
                      </tr>
                    ))
                  )}
                </tbody>
                ...
                <tbody>
                  {filteredGroupedPrices.length > 0 ? (
                    filteredGroupedPrices.map((group, index) => {
                      const totalPrice = group.productos.reduce((sum, product) => sum + parseFloat(product.precio_promedio || 0), 0);
                      const averagePrice = group.productos.length > 0 ? totalPrice / group.productos.length : 0;
                      return (
                        <tr key={`${group.region_id}-${group.group_id}-${index}`}>
                          <td>{group.region || 'Sin región'}</td>
                          <td>{group.grupo || 'Sin grupo'}</td>
                          <td style={{ textAlign: 'right' }}>{averagePrice > 0 ? formatPrice(averagePrice) : 'N/A'}</td>
                        </tr>
                      );
                    })
                  ) : (
                ...
                <tbody>
                  {brandStats.length > 0 ? (
                    brandStats.map((brand, index) => (
                      <tr key={`${brand.region || 'region'}-${brand.brand}-${index}`}>
                        <td>{brand.region || 'Sin región'}</td>
                        <td>{brand.brand || 'Sin marca'}</td>
                        <td style={{ textAlign: 'right', fontFamily: 'monospace' }}>{formatPrice(brand.average_price)}</td>
                        <td style={{ textAlign: 'right' }}>{brand.product_count || 0}</td>
                        <td style={{ textAlign: 'right', color: brand.vs_average_percentage > 0 ? '#d32f2f' : brand.vs_average_percentage < 0 ? '#388e3c' : '#1976d2', fontFamily: 'monospace' }}>
                          {brand.vs_average_percentage > 0 ? '+' : ''}{brand.vs_average_percentage}%
                        </td>
                      </tr>
                    ))
                  ) : (
            ...
            <div style={{
              ...
              {topAndBottomBrands.length > 0 ? (
                <div style={{ width: '100%', height: '100%' }}>
                  <Bar
                    data={{
                      labels: topAndBottomBrands.map(brand => brand.brand || 'Sin nombre'),
                      datasets: [
                        {
                          label: 'Precio Promedio',
                          data: topAndBottomBrands.map(brand => brand.average_price || 0),
                          backgroundColor: [
                            'rgba(255, 99, 132, 0.6)',
                            'rgba(54, 162, 235, 0.6)',
                            'rgba(255, 206, 86, 0.6)',
                        ...
                        tooltip: {
                          callbacks: {
                            label: function (context) {
                              const brand = topAndBottomBrands[context.dataIndex];
                              return [
                ...
                <div style={{ textAlign: 'center', color: '#666' }}>
                  {loading ? 'Cargando datos del gráfico...' : 'No hay suficientes datos para mostrar el gráfico'}
                </div>
              )}
            </div>
          </div>
