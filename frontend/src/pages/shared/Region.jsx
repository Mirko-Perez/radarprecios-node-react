...
const Region = () => {
    ...
    useEffect(() => {
        const loadRegionAndStore = async () => {
            ...
            const { selectedRegion: regionId, selectedStore: storeId, regionName, storeName } = location.state;
    ...
    useEffect(() => {
        ...
        if (!isFromCheckIn) {
            if (selectedRegion) {
                setLoading(true);
                axios.get(`${API_URL}/stores/region/${selectedRegion.id}`)
                    .then(response => {
    ...
    useEffect(() => {
        if (selectedStore) {
            ...
            axios.get(`${API_URL}/brands?store_id=${selectedStore.value}`)
                .then(response => {
    ...
    const handleBrandChange = (selectedOption) => {
        ...
        if (selectedOption) {
            ...
            axios.get(`${API_URL}/products/brand/${selectedOption.value}`)
                .then(response => {
    ...
    const handleSavePrice = async () => {
        ...
        try {
            ...
            console.log('Sending price data:', {
                ...
                price: price,
                quantity: selectedBrand && selectedBrand.label === 'Fritz' ? quantity : null,
    ...
    <div className="region-container">
        ...
        <div className="region-selector" style={{ marginBottom: '20px' }}>
                ...
                <Select
                    ...
                    onChange={(option) => setSelectedRegion(option ? regiones.find(r => r.id === option.value) : null)}
            ...
            <div className="store-selector" style={{ marginBottom: '20px' }}>
                ...
                <Select
                    ...
                    placeholder={loading ? 'Cargando comercios...' : 'Selecciona un comercio'}
            ...
            <div className="brand-selector" style={{ marginBottom: '20px' }}>
                ...
                <Select
                    ...
                    placeholder={brandsLoading ? 'Cargando marcas...' : 'Selecciona una marca'}
            ...
            <div className="product-selector" style={{ marginBottom: '20px' }}>
                ...
                <Select
                    options={products.map(product => ({
                        ...
                        image: product.imagen ? `${API_URL}${product.imagen}` : '/default-product.png'
                    }))}
                    value={selectedProduct}
                    onChange={handleProductChange}
                    placeholder={productsLoading ? 'Cargando productos...' : 'Selecciona un producto'}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            ...
                            {selectedBrand && selectedBrand.label === 'Fritz' && (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <label style={{ fontWeight: 'bold', minWidth: '80px' }}>Cantidad:</label>
                                    <input
                                        type="number"
                                        value={quantity}
                            ...
                            {/* Precio */}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <label style={{ fontWeight: 'bold', minWidth: '80px' }}>Precio:</label>
                                <input
                                    type="number"
                                    value={price}
                            ...
                            <div style={{ marginTop: '10px' }}>
                                <button
                                    ...
                                    disabled={!price || isSaving || (selectedBrand?.label === 'Fritz' && !quantity)}
                                    style={{
                                        backgroundColor: !price || isSaving || (selectedBrand?.label === 'Fritz' && !quantity) ? '#ccc' : '#4CAF50',
                                        color: 'white',
                                        border: 'none',
                                        padding: '10px 20px',
                                        borderRadius: '4px',
                                        cursor: !price || isSaving || (selectedBrand?.label === 'Fritz' && !quantity) ? 'not-allowed' : 'pointer',
                                        fontWeight: 'bold',
                                        width: '200px',
                                        fontSize: '16px'
                                    }}
        ...
        <div className="observation-container" style={{ margin: '30px 0', padding: '20px', border: '1px solid #e0e0e0', borderRadius: '8px' }}>
            ...
            <button
                    ...
                    try {
                        ...
                        if (response.data && response.data.success) {
                            setObservationMessage({
                                text: response.data.message || 'Observación guardada correctamente',
                                isError: false
                            });
                ...
                style={{
                    backgroundColor: !observation.trim() || isSavingObservation ? '#cccccc' : '#4CAF50',
                    color: 'white',
                    border: 'none',
                    padding: '10px 20px',
                    borderRadius: '4px',
                    cursor: !observation.trim() || isSavingObservation ? 'not-allowed' : 'pointer',
                    fontWeight: 'bold',
                    fontSize: '14px'
                }}
            >
